...
m_rat.a_exp    = [1 1 1 1 1];
m_rat.a_signal_type      = [4 3 4 3 3];
m_rat.a_regress    = [0 0 0 1 0];
% tels if there is some non linearity like (y(k-a)^b)*(y(k-c)^d)
% u = 2 y=1 none =0
m_rat.b_signal_type = [0 0 3 3 0];
% tels the d param
m_rat.b_exp = [0 0 1 1 0];
% tels the C param
m_rat.b_regress = [0 0 1 0 0];
...
for k=3:N
    e(k)=r(k)-y(k-1);
    u(k-1)=(mtheta(1)*r(k-1)^m_rat.a_exp(1)+mtheta(2)*y(k-1)^m_rat.a_exp(2)+mtheta(3)*y(k-2)^m_rat.a_exp(3)*r(k-1)^m_rat.b_signal_type(3)+mtheta(4)*y(k-2)^m_rat.a_exp(4)*y(k-1)^m_rat.b_signal_type(4))/(1+mtheta(5)*y(k-1)^m_rat.a_exp(5));
